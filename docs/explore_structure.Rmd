...
targets::tar_load(games_included)
...
```{r prepare-and-verify-complete-data-full}
...
```{r explore-on-complete-data-full}
...
```{r visualize-complete-data, fig.height=20, fig.width=9}
loadings_struct %>% 
...
```{r prepare-and-verify-data-valid}
...
```{r visualize-data-valid, fig.height=20, fig.width=8}
loadings_struct %>% 
  arrange(across(starts_with("MR"), ~ (abs(.x) > 0.4) * abs(.x))) %>% 
  mutate(index = as_factor(index)) %>% 
  pivot_longer(-index, names_to = "factor", values_to = "loading") %>% 
...
```{r prepare-and-verify-data-normal}
...
```{r visualize-data-normal, fig.height=20, fig.width=8}
loadings_struct %>% 
  arrange(across(starts_with("MR"), ~ (abs(.x) > 0.4) * abs(.x))) %>% 
  mutate(index = as_factor(index)) %>% 
  pivot_longer(-index, names_to = "factor", values_to = "loading") %>% 
